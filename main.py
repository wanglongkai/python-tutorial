...
@app.get('/hello/{name}')
async def hello(name: str = Path(..., min_length=3, max_length=10, description="The name of the person")):
    return {"message": f"Hello {name}"}

# 路径参数
@app.get('/items/{item_id}')
async def get_item(item_id: int = Path(..., description="The ID of the item", gt=0, lt=101)):
    return {"item_id": item_id}

# 查询参数: 直接在处理函数中添加Query参数
@app.get('/news')
async def get_news(
    q: str = Query('value', description="The query string", min_length=3, max_length=10, pattern="^[a-zA-Z0-9]+$"), 
